...
function pickImageUrl(prompt = "") {
  ...
  if (
    /(kids|children|child|explorer|learning|education|educational|toys|toy|adventure|play|books|school|preschool)/.test(
      text,
    )
  ) {
    return imageMap.kids;
  }
...
export function ensureImagesInGeneratedCode(appCode, prompt) {
  ...
  const altText = /(staffing|recruit|hiring|company|office|agency|career|jobs)/i.test(
    ...
    : /(kids|children|child|explorer|learning|education|educational|toys|toy|adventure|play|books|school|preschool)/i.test(
          prompt,
        )
      ? "Children exploring and learning through play"
