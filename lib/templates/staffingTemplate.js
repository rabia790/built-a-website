  ...
  return (
    <header className="site-header">
      <button type="button" className="brand" onClick={() => setCurrentPage("home")}>
        <span data-edit-id="brand-logo" className="brand-logo-mark">{getInitials(brandName)}</span>
        <span data-edit-id="brand-name" className="brand-name">{brandName}</span>
      </button>
...
export default App;`;
  ...
  --dark: #0f172a;
  --hero-headline-size: clamp(4rem, 8vw, 8rem);
...
.hero-section {
  ...
  align-items: center;
  min-height: min(920px, calc(100vh - 4.5rem));
...
h1 { max-width: 12ch; font-size: clamp(4rem, 8vw, 8rem); }
.hero-title {
...
@media (max-width: 1050px) {
  ...
  :root {
    --hero-headline-size: clamp(3rem, 10vw, 5.5rem);
  }
...
@media (max-width: 720px) {
  ...
  :root {
    --hero-headline-size: clamp(2.75rem, 13vw, 4.75rem);
  }
  h1 { font-size: var(--hero-headline-size); }
  .hero-title {
    line-height: 0.96;
